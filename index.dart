...
class User {
  ...
  User(this.username, this.age, this.gender, this.password,
      this.enrolledCourseIds);
}

void main() {
  ///store data from excel to user class
  List<User> users = [];
  List<Course> courses = [];

  String userString = File('./userData.json').readAsStringSync();
  List<dynamic> jsonData = json.decode(userString);
  
  String courseString = File('./courses.json').readAsStringSync();
  List<dynamic> coursejson= json.decode(courseString);
  ...
  for (var item in jsonData) {
    ...
    int age = item['age'];
    List<dynamic> dynamicList = item['enrolledCourseIds'] ?? [];
    List<String> stringList =
        dynamicList.map((element) => element.toString()).toList();
    users.add(User(username, age, gender, password, stringList));
  }

 for (var course in coursejson) {
   String id=course['id'];
   String name=course['name'];
   String instructor=course['instructor'];
   List<dynamic> dynamicList = course['courseMaterials'] ?? [];
   List<String> courseMaterials=dynamicList.map((element) => element.toString()).toList();
   List<dynamic> dynamicList2 = course['quizzes'] ?? [];
   List<String> quizzes=dynamicList2.map((element) => element.toString()).toList();   
   Map<String, dynamic> dynamicList3 = course['grades'] ?? 0;
   Map<String, int> grades=dynamicList3.map((key, value) => MapEntry(key, value as int));
  
    courses.add(Course(id,name,instructor,courseMaterials,quizzes,grades));
  }
...
users.map((element) => print(element.toString()));
courses.map((element) => print(element.toString()));
...
void register(List<User> users) {
  ...
  if (user) {
    ...
    users.add(User(username, age, gender, password, []));
